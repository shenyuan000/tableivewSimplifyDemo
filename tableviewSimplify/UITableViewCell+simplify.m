...
+ (void)initialize {
    HsCellDefaultFontForTitleView = [[[HsThemeManager shareInstance] readConfigFromConfigFile:@"defaultTitleFont" defaultValue:@(15)] floatValue];
    HsCellDefaultFontForDetailView = [[[HsThemeManager shareInstance] readConfigFromConfigFile:@"defaultDetailFont" defaultValue:@(14)] floatValue];
}
...
- (void)setBaseViewController:(HsBaseViewController *)baseViewController {
    objc_setAssociatedObject(self, tableKeyForBaseViewController, baseViewController, OBJC_ASSOCIATION_ASSIGN);
}
- (HsBaseViewController *)baseViewController {
